...
function BestHistory() {
  ...
  const [listAnime, setListAnime] = React.useState<Array<Anime>>([state.ListBestAnimation[0], state.ListBestAnimation[1], state.ListBestAnimation[2]]);
    ...
    <C.ContainerAnime key={index}>
     {index === 0 ?  "3° Lugar": index === 1 ? "1° Lugar" : "2° Lugar"}
      ...
      <div>
        ...
        {state.ListBestAnimation[index]?.attributes?.episodeCount !== 0 && (
          <C.AnimeEpsCount >
            Número de episódios: {state?.ListBestAnimation[index]?.attributes?.canonicalTitle === "One Piece" ? '1047' : state?.ListBestAnimation[index]?.attributes?.episodeCount}
          </C.AnimeEpsCount>
        )}
